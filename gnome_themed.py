...
class GnomeThemed(Gtk.Window):
    def __init__(self):
        ...
        self.panel_btn.connect("color-set", self.on_button_clicked, "#panel")
        self.popup_btn = Gtk.ColorButton()
        self.panel_btn.connect("color-set", self.on_button_clicked, ".candidate-popup-boxpointer")
    ...
    def on_button_clicked(self, widget, setting):
        color = themer.HexColor(self.panel_btn.get_rgba())
        self.hex_color = color.convert()
        themer.change_setting(setting, self.hex_color)
